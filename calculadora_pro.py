...
def make_display(root) -> tk.Entry:
    ...
    display.grid(row=1, column=0, columnspan=5, sticky='news', pady=(0, 20))
    display.config(
...
def _display_control_a(event):
    event.widget.selct_range(0, 'end')
...
def make_buttons(root) -> List[List[tk.Button]]:
    ...
    for row, row_value in enumerate(button_texts, start=2):
        ...
        for col_index, col_value in enumerate(row_value):
            boton = tk.Button(root, text=col_value)
            boton.grid(row=row, column=col_index, sticky='news', padx=5, pady=5)
            boton.config(
                font=('Heveltica', 15, 'normal'),
                pady=40, width=1, background='#f1f2f3', bd=0,
                cursor='hand2',highlightthickness=0,
                highlightcolor='#ccc', activebackground='#ccc',
                highlightbackground='#ccc'
            )            
            button_row.append(boton)
